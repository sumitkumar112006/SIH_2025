// KMRL AI Assistant Interface - UI and Interaction Management
class KMRLAIAssistant {
    ...
    async sendMessage(message = null) {
        ...
        try {
            // Get AI response using separated API client
    ...
    async getAIResponse(userMessage) {
        // Create system prompt using the separated client
        const systemPrompt = this.openaiClient.createSystemPrompt({
            purpose: this.projectContext.purpose,
            features: this.projectContext.features,
            userRole: `${this.currentUser.role} (${this.currentUser.name})`
        });

        // Get response from OpenAI client
        const result = await this.openaiClient.getContinuationResponse(
            userMessage,
            this.chatHistory,
            systemPrompt,
            {
...
Please ask me specific questions about any of these topics, and I'll provide detailed assistance.`;
    ...
    // Document analysis method using the API client
    async analyzeDocument(documentContent, analysisType = 'summary') {
        try {
            const result = await this.openaiClient.analyzeDocument(documentContent, analysisType);
    ...
    // Tender analysis method using the API client
    async analyzeTender(tenderData, analysisType = 'summary') {
        try {
            const result = await this.openaiClient.analyzeTender(tenderData, analysisType);
            return result.success ? result.content : null;
        } catch (error) {
    ...
    async testApiConnection() {
        try {
            const result = await this.openaiClient.testConnection();
            this.updateStatus(result.message, result.success ? 'success' : 'error');
    ...
    updateApiKey(newApiKey) {
        this.openaiClient.updateApiKey(newApiKey);
        this.updateStatus('API key updated', 'success');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    // Make sure OpenAI client is loaded first
    if (typeof OpenAIClient !== 'undefined') {
        window.aiAssistant = new KMRLAIAssistant();
    } else {
        console.error('OpenAIClient not found. Make sure to include openai-client.js before ai-assistant-interface.js');
    }
});

...
class LoginManager {
    ...
    init() {
        ...
        // Generated OTP for demo (normally from backend)
        this.currentOTP = this.generateOTP();
        console.log(`Demo OTP: ${this.currentOTP}`); // For testing purposes

        // Add demo credentials helper buttons
    ...
    setupEventListeners() {
        ...
        // 2FA form
        document.getElementById('twoFactorForm').addEventListener('submit', (e) => this.handleTwoFactorVerification(e));

        // Forgot password form
        document.getElementById('forgotPasswordForm').addEventListener('submit', (e) => this.handleForgotPassword(e));

        // Modal controls
        document.getElementById('resendCode').addEventListener('click', () => this.resendOTP());
        ...
        document.getElementById('twoFactorModal').addEventListener('click', (e) => {
            if (e.target.id === 'twoFactorModal') this.closeTwoFactorModal();
        });
    ...
    async handleLogin(e) {
        ...
        try {
            ...
            this.showNotification('Login successful! Sending verification code...', 'success');
    ...
    async handleTwoFactorVerification(e) {
        ...
        if (enteredOTP.length !== 6) {
            this.showOTPError('Please enter all 6 digits');
            return;
        }

        // Show loading state
        this.setVerifyButtonState(true);
        ...
        try {
            ...
            this.showNotification('Verification successful! Redirecting...', 'success');
            ...
            this.setVerifyButtonState(false);
        }
    }
    ...
    async resendOTP() {
        ...
        try {
            ...
            // Generate new OTP
            this.currentOTP = this.generateOTP();
            console.log(`Resent OTP: ${this.currentOTP}`);
    ...
    startResendTimer() {
        ...
        this.resendTimer = setInterval(() => {
            this.resendTimeLeft--;
            timerElement.textContent = `Resend code in ${this.resendTimeLeft}s`;
        ...
        if (loginCard) {
            ...
            demoSection.innerHTML = `
                <div class="demo-header">
                    ...
                    <p>Click any button below to auto-fill credentials</p>
                </div>
                <div class="demo-buttons">
                    <button type="button" class="btn btn-sm btn-secondary" onclick="fillDemoCredentials('admin')">
                        Admin Login
                    </button>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="fillDemoCredentials('manager')">
                        Manager Login
                    </button>
                    <button type="button" class="btn btn-sm btn-secondary" onclick="fillDemoCredentials('staff')">
                        Staff Login
                    </button>
                </div>
            `;

            // Insert before login footer
            const loginFooter = document.querySelector('.login-footer');
            if (loginFooter) {
                loginCard.insertBefore(demoSection, loginFooter);
            } else {
